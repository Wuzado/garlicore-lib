Contributing to Garlicore
=======

We're working hard to make *garlicore* the most powerful JavaScript library for working with garlicoin. Our goal is to have *garlicore* be a library that can be used by anyone interested in garlicoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in garlicore that any change must adhere.
...
When possible, garlicore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Garlicore methods. For example, calling `JSON.stringify` on a Garlicore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'garlicore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the garlicore object.

DO:
```javascript
var garlicore = require('../');
var PublicKey = garlicore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add garlicoin-project git@github.com:garlicoin-project/garlicore.git
git pull --rebase garlicoin-project master
```
...
```
Finally go to [github.com/garlicoin-project/garlicore](https://github.com/garlicoin-project/garlicore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of garlicore.

If you have any questions feel free to post them to
[github.com/garlicoin-project/garlicore/issues](https://github.com/garlicoin-project/garlicore/issues).
