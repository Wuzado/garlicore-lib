...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new tuxcore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new tuxcore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = tuxcore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new tuxcore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function tuxcoreTest() {
      block1 = tuxcore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('tuxcore', tuxcoreTest, {maxTime: maxTime});
